...
class DelegateToAgentFunction(Function):
    ...
    # Initialize with optional references to the calling agent and resume GUIDs.
    # Builds the agent_config map linking agent names to their planning prompts and classes.
    # @param calling_agent: The parent agent instance (used to extract child resume GUIDs on resumption).
    # @param parent_resume_guid: The parent agent's GUID when resuming after a permission pause.
    # @param child_resume_guid: The child agent's GUID to resume directly (skips planning).
    ...
    # Inject new_content immediately after the opening <context> tag in text.
    # @param text: String containing a <context> tag.
    # @param new_content: Content to insert after the tag.
    # @returns: Modified string.
    def add_between_context_tags(text, new_content):
        return text.replace("<context>", f"<context>{new_content}", 1)

    # Run the full delegation workflow: optionally plan via PlannerAgent, then execute via
    # the target agent class. Handles resumption (skips planning when child_resume_guid is set).
    # Re-raises ToolPermissionRequiredException with the child's GUID attached so the
    # external interface can resume the child agent later.
    # @param args: Dict with 'agent_name', 'task_description', optional 'task_context'.
    # @returns: Dict with 'status', 'agent_name', 'result', 'child_resume_guid', and plan/exec
    #           metadata, or 'error' on planning/execution failure.
